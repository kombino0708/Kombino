...
const contactInfo = {
    phone_numbers: [
        {
            number: "7986669220",
            formatted: "+91 7986669220",
            display: "Call us: +91 7986669220"
        },
        {
            number: "8264146204", 
            formatted: "+91 8264146204",
            display: "Call us: +91 8264146204"
        },
        {
            number: "7009560208",
            formatted: "+91 7009560208", 
            display: "Call us: +91 7009560208"
        }
    ],
    email: "kombinofoods@gmail.com",
    location: "Tricity (Mohali, Chandigarh, Panchkula)"
};
...
// Load user data from localStorage (but don't use it due to restrictions)
function loadUserData() {
    // Note: localStorage is not available in sandbox, so we'll use in-memory storage
    console.log('User data handling initialized (in-memory only)');
}

// Save user data (in-memory only due to sandbox restrictions)
function saveUserData() {
    // Note: localStorage is not available in sandbox
    console.log('User data saved (in-memory only)');
}
...
function simulateEmail(type, data) {
    ...
    if (type === 'signup') {
        ...
        console.log(`Contact numbers available: ${contactInfo.phone_numbers.map(p => p.formatted).join(', ')}`);
    }
}
...
function initializePhoneLinks() {
    // Add click analytics and feedback for all phone links
    document.querySelectorAll('a[href^="tel:"]').forEach(phoneLink => {
...
// Initialize phone links after DOM is ready
document.addEventListener('DOMContentLoaded', function() {
...
if ('requestIdleCallback' in window) {
    requestIdleCallback(() => {
        ...
        // Log contact information for verification
        console.log('Contact numbers configured:', contactInfo.phone_numbers.map(p => p.formatted).join(', '));
    });
}

console.log('Kombino: Complete endorsements website with authentication and checkout loaded successfully! 🌿');
console.log('All three phone numbers are now available:', contactInfo.phone_numbers.map(p => p.formatted).join(', '));